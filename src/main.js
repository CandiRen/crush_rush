import { BOARD_SIZE, findHintMove } from "./board";
import { Game } from "./game";
import { LEVELS } from "./levels";
import { BASE_MISSIONS, createInitialMissionState, updateMissionProgress, formatMissionProgress, resetMissionState, claimMissionReward } from "./missions";
import { addSoftCurrency, ensureLevelProgress, loadProfile, recordLevelCompletion, saveProfile } from "./profile";
...
let missionState = initializeMissionState();
let profileState = ensureLevelProgress(loadProfile(PROFILE_STORAGE_KEY), LEVELS.length);
persistProfileState();
let dragState = null;
...
function renderSelectedLevelDetail() {
    ...
    menuLevelsList.append(header, description, metricsList, objectivesHeading, objectivesList, progressBlock, actions);
}
function selectMenuLevel(index) {
    if (index < 0 || index >= LEVELS.length) {
        return;
    }
...
function countActiveCells(layout) {
    if (!layout) {
        return 0;
    }
...
function renderLevelMap() {
    ...
    if (currentView === "menu") {
        window.requestAnimationFrame(() => {
            const targetNode = menuMapPath.querySelector(".map-node.selected") ??
                menuMapPath.querySelector(".map-node.current");
            if (!targetNode) {
                return;
            }
            const container = menuMapScroll;
            const targetTop = targetNode.offsetTop - container.clientHeight / 2 + targetNode.offsetHeight / 2;
            container.scrollTo({
...
function handleLevelCompletion() {
    ...
    const { profile: updatedProfile, unlockedLevels } = recordLevelCompletion(profileState, level.id, stars, score, LEVELS.length, combo, efficiency);
...
function showSessionSummary({ stars, score, combo, efficiency, previousBest, level, unlockedLevels }) {
...
function triggerSummaryConfetti() {
...
function extractNewlyCompletedMissions(previous, current) {
...
function showMissionToast(missions) {
...
function clearHintSuggestion() {
    if (!hintSuggestion) {
        return;
    }
    hintSuggestion = null;
    const hintedTiles = boardElement.querySelectorAll(".tile.hint-swap, .tile.hint-match");
...
function render() {
    ...
    const boosterTargets = activeBooster === "free-switch" && boosterSelection
    ...
    renderBoard(boardSnapshot, status, jellyGrid, crateGrid, {
        ...
        hintSwap: hintSuggestion && status === "playing"
...
function renderHud() {
    ...
    if (scorePerMove > 0) {
        items.push(createHudItem("Skor/Langkah", scorePerMove.toLocaleString("id-ID", {
            maximumFractionDigits: 1,
            minimumFractionDigits: 1
        })));
    }
    if (game.hasJellyTarget()) {
        items.push(createHudItem("Jelly", game.getJellyRemaining().toLocaleString("id-ID")));
    }
    if (game.hasCrateTarget()) {
        items.push(createHudItem("Crate", game.getCrateRemaining().toLocaleString("id-ID")));
    }
    hud.append(...items);
}
function renderBoosters(status) {
    const inventory = game.getBoosters();
    Object.keys(boosterButtons).forEach((type) => {
        ...
        const disabled = status !== "playing" || tutorialActive || isAnimating || available <= 0;
...
function createBoosterButton(type, counterElement) {
...
function createHudItem(label, value) {
...
function createMenuStatBlock(label, valueElement) {
    const container = document.createElement("div");
...
function renderBoard(board, status, jellyGrid, crateGrid, options = {}) {
    ...
    const boosterSelectionKey = options.boosterSelection
        ...
        : null;
    const boosterTargetKeys = new Set(options.boosterTargets?.map((pos) => positionKey(pos)) ?? []);
...
function decorateTile(container, tile) {
...
function getInteractionBlockMessage() {
    if (tutorialActive) {
...
function cleanupDragState(releaseCapture = true) {
...
function clearTileAnimation(element) {
...
function handleBoosterTileClick(position) {
    if (!activeBooster) {
        return;
    }
    const blockMessage = getInteractionBlockMessage();
...
function handleBoosterResult(booster, result) {
...
function handleBoosterFailure(booster, reason) {
    const label = BOOSTER_INFO[booster].label;
    let message;
    switch (reason) {
        case "no-charge":
...
function onTilePointerDown(position, event) {
    if (!event.isPrimary) {
        return;
    }
...
function updateDragVisual(deltaX, deltaY) {
    if (!dragState) {
        return;
    }
    const element = dragState.element;
...
function attemptDirectSwap(start, target) {
    registerPlayerAction();
...
function onTileClick(position, event) {
    if (activeBooster) {
        event?.preventDefault();
        suppressNextClick = false;
...
async function processSwap(start, target) {
    if (swapInProgress) {
        return;
    }
    const blockMessage = getInteractionBlockMessage();
...
function playSwapPreview(start, target, options) {
...
function handleSuccessfulSwap(result) {
...
function startPlayback(frames, finalMessage) {
    if (frames.length === 0) {
        infoMessage = finalMessage;
        render();
        return;
...
function loadMissionStateFromStorage() {
    try {
...
function persistMissionState(state) {
    try {
...
function buildMissionItem(mission, progress) {
...
function applyMissionDelta(delta) {
...
function handleMissionClaim(mission) {
    const progress = missionState.progress[mission.id];
    if (!progress?.completed || progress.claimed) {
        return;
    }
...
function formatSpecialList(labels) {
    if (labels.length === 0) {
        return "";
    }
    if (labels.length === 1) {
