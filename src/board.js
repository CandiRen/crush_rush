...
function createTile(kind, special) {
...
function createsMatchAt(board, row, col, kind) {
    // Check horizontal
...
function collectEntireBoard(board, accumulator) {
...
function applyColorBombEffect(board, origin, targetKind, accumulator) {
...
function clearMatches(board, matches, swapContext) {
    const uniqueRemoval = new Map();
    const createdSpecials = [];
    const swapPriority = new Set(swapContext ? [positionKey(swapContext.primary), positionKey(swapContext.secondary)] : []);
    const matchSpecialPlans = [];
...
function determineSpecialCreation(board, group, swapPriority) {
...
function chooseSpecialPlacement(positions, swapPriority) {
...
function applySpecialEffect(board, origin, special, accumulator) {
...
function expandSpecialChain(board, accumulator) {
    const processed = new Set();
    const queue = Array.from(accumulator.values());
    while (queue.length > 0) {
        const pos = queue.pop();
        const key = positionKey(pos);
...
function triggerSpecialCombo(board, a, b) {
...
export function ensurePlayableBoard(board) {
    let attempts = 0;
...
function evaluateHintMove(board, a, b) {
    ...
    const involvesSwap = (positions) => positions.some((pos) => (pos.row === a.row && pos.col === a.col) || (pos.row === b.row && pos.col === b.col));
...
function wouldFormMatch(board, a, b) {
...
function executeSwap(board, a, b, options) {
    if (!inBounds(board, a) || !inBounds(board, b)) {
        return {
            valid: false,
    ...
    if (comboResult) {
        ...
        const comboScore = Math.round(comboRemovalCount * BASE_MATCH_SCORE * cascadeIndex * SPECIAL_COMBO_MULTIPLIER);
...
export function attemptSwap(board, a, b) {
    return executeSwap(board, a, b, { requireMatch: true });
}
export function forceSwap(board, a, b) {
    return executeSwap(board, a, b, { requireMatch: false });
}
export function hammerTile(board, position) {
